		...
		if clean_file_path.endswith("_clean.wav"):
			...
			if controls:
				...
				for ef in effected_files:
					...
					dist_splits = np.array([df_data[i*frame_size:(i+1)*frame_size] for i in range(int(0.02 * (df_data.shape[0]/frame_size)))])
					clean_splits = np.array([cf_with_controls[i*frame_size:receptive_field+(i+1)*frame_size, :] for i in range(int(0.02 * ((cf_with_controls.shape[0]-receptive_field)/frame_size)))])
